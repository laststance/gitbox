...
test.describe('Keyboard Navigation', () => {
  ...
  test('. (period) key should toggle overflow menu on focused card', async ({ page }) => {
    // Given: カードにフォーカスがある
  ...
  test('Enter key should execute default action on focused card', async ({ page }) => {
    // Given: カードにフォーカスがある
    const card = page.locator('[data-testid="repo-card"]').first();
    await card.focus();
  ...
  test('Keyboard navigation should be accessible (WCAG AA)', async ({ page }) => {
    // Given: ページが読み込まれている
